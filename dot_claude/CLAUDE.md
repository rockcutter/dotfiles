...
- 変換例:
  ...
  - バグを潰す → バグを修正する
  - メソッドが生える → メソッドが追加される
- URLの前後には半角スペースを挟み、全角文字をURLに隣接させない
...
## fork subagent
- Agentツールの subagent_type="fork" で、会話コンテキストを全継承した分身を
  バックグラウンド起動できる（モデルは親と同じ）
- 使う場面: 独立サブタスクの並列実行 / 会話の文脈前提の作業の続き /
  大量読み込みをメインのコンテキストに入れたくないとき
- 同一ファイルを複数forkに同時編集させない
